...
var callAPI = function(url) {
	...
	if (url === urlGuardian) {
		webTitle = x.response.results[i].webTitle;
		sectionName = x.response.results[i].sectionName;
		thumbnail = x.response.results[i].fields.thumbnail
		impressionsValue = x.response.results[i].fields.wordcount
	} else if (url === urlUSA) {
		webTitle = x.articles[i].title;
		sectionName = "sectionName";
		thumbnail = x.articles[i].urlToImage;
		impressionsValue = x.articles[i].publishedAt.substring(5,10);
	} else {
		webTitle = x.results[i].title;
		sectionName = x.results[i].section;
		thumbnail = x.results[i].multimedia[0].url;
		impressionsValue = x.results[i].updated_date.substring(5,10);
	}
	...
	$("h3").click(function () {
		...
		if (url === urlGuardian) {
			...
			var description = x.response.results[index].blocks.requestedBodyBlocks;
